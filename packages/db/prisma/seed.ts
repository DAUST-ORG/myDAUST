...
if (dbUrl.includes("daust-prod") && process.env.SEED_ALLOW_PROD !== "1") {
  console.error(
    "Refusing to demo-seed a daust-prod database. Set SEED_ALLOW_PROD=1 to override (don't).",
  );
  process.exit(1);
}
...
const STUDENTS: StudentSpec[] = [
  ...
  },
  {
    ...
    total: 3_500_000,
    installments: [
      { sequence: 1, dueDate: "2026-09-30", amountDue: 3_500_000 },
    ],
  },
];
...
async function seedFacultyProfiles() {
  ...
  console.log(
    `Seeded ${FACULTY_PROFILES.length} faculty profiles (${madePublic.size} public, 1 private).`,
  );
}
...
async function seedStandardPackages() {
  ...
  const tuition = schedule.rows.reduce(
    (sum, row) => sum + row.amountTuitionXof,
    0,
  );
  const housing = schedule.rows.reduce(
    (sum, row) => sum + row.amountHousingXof,
    0,
  );
  const cafeteria = schedule.rows.reduce(
...
async function seedAcademics() {
  ...
  const fall = await prisma.term.findUniqueOrThrow({
    where: { name: "Fall 2026" },
  });
  ...
  async function section(
    ...
    return prisma.section.upsert({
      where: {
        courseId_termId_sectionCode: { courseId, termId, sectionCode: code },
      },
      update: {
        capacity,
        days,
  ...
  const csc101Spring = await section(
    "CSC 101",
    spring.id,
    "A",
    30,
  ...
  await prisma.enrollment.upsert({
    where: {
      studentId_sectionId: {
        studentId: "stu_demo_aissatou",
        sectionId: csc101Spring.id,
      },
    },
  ...
  const mth210Fall = await section(
    "MTH 210",
    fall.id,
    "A",
    30,
  ...
  const eng250Fall = await section(
    "ENG 250",
    fall.id,
    "A",
    25,
  ...
  console.log(
    "Seeded academics: 1 program, 5 courses, Fall 2026 sections (CSC 201 cap=2).",
  );
...
async function seedInsightsDemo(ce201Id: string, programId: string) {
  ...
  const hw1 = await prisma.assignment.findFirst({
    where: { sectionId: ce201Id, title: "HW1 — Boolean Algebra" },
  });
  ...
  for (const c of cohort) {
    ...
    const enr = await prisma.enrollment.upsert({
      where: {
        studentId_sectionId: { studentId: student.id, sectionId: ce201Id },
      },
      update: {},
  ...
  console.log(
    `Seeded insights demo: ${cohort.length}-student cohort in CE 201 with attendance + grades.`,
  );
}
...
async function seedAssignments(
  ...
  for (const d of defs) {
    const existing = await prisma.assignment.findFirst({
      where: { sectionId: d.sectionId, title: d.title },
    });
  ...
  const mamadouCe201 = await prisma.enrollment.findFirst({
    where: {
      sectionId: ce201Id,
      student: { person: { firstName: "Mamadou" } },
    },
  });
  if (hw1 && mamadouCe201) {
    await prisma.submission.upsert({
...
async function seedAnnouncements() {
  const items = [
    {
      title:
        "Le Sénégal décroche la Lune: Pourquoi 2026 marque un tournant historique pour l'innovation africaine",
      body: "Le \"Sputnik\" de l'Afrique de l'Ouest Le 2 avril 2026 restera dans l'histoire comme le jour où le Sénégal est devenu la première nation africaine à poser un instrument scientifique sur la Lune.",
      category: "Projects",
      audience: "all",
      author: "DAUST Press",
    },
    {
      title: "DAUST Career Fair 2026: Shaping Futures, Creating Opportunities",
      body: "On March 28, the DAUST campus in Somone came alive as the Career Fair 2026 brought together over 60 employers and hundreds of students.",
    ...
    },
    {
      ...
      body: "Add/drop closes two weeks after term start. Register early — popular sections fill fast.",
      category: "Registrar",
      audience: "all",
      author: "Registrar",
    },
    {
      title: "Tuition installment 1 due Sept 15",
      body: "Pay via Wave, Orange Money, or card from the Billing page.",
      category: "Bursar",
      audience: "student",
      author: "Bursar Office",
    },
    {
      title: "Library extended hours",
      body: "The library is open until midnight during finals week.",
      category: "Library",
      audience: "all",
      author: "Library",
    },
    {
      ...
      body: "Submit Fall midterm grades by the end of week 8.",
      category: "Academics",
      audience: "faculty",
      author: "Registrar",
    },
...
async function seedFinanceMgmt() {
    ...
    await prisma.expense.createMany({
      data: [
        ...
        },
        {
          ...
          amount: 1_200_000,
          isEstimate: false,
          incurredOn: new Date("2026-05-01"),
        },
        {
          costCenterCode: "1100",
          category: "Salary",
          ...
          amount: 1_200_000,
          isEstimate: false,
          incurredOn: new Date("2026-04-01"),
        },
        {
          costCenterCode: "1100",
          category: "Salary",
          description: "Faculty payroll (estimated)",
          amount: 18_000_000,
          isEstimate: true,
          incurredOn: new Date("2026-06-01"),
        },
        {
          costCenterCode: "3300",
          category: "Salary",
          description: "Admin staff payroll (estimated)",
          amount: 9_000_000,
          isEstimate: true,
          incurredOn: new Date("2026-06-01"),
        },
        {
          ...
          amount: 4_500_000,
          isEstimate: false,
          incurredOn: new Date("2026-06-10"),
        },
        {
          costCenterCode: "3500",
          category: "IT",
          description: "Cloud + software licenses",
          amount: 2_200_000,
          isEstimate: false,
          incurredOn: new Date("2026-06-12"),
        },
        {
          costCenterCode: "2200",
          category: "Operations",
          description: "Career Fair logistics",
          amount: 3_000_000,
          isEstimate: false,
          incurredOn: new Date("2026-06-15"),
        },
        {
          costCenterCode: "3100",
          category: "Operations",
          description: "Bank / PayTech fees",
          amount: 350_000,
          isEstimate: false,
          incurredOn: new Date("2026-06-20"),
        },
      ],
    });
  }
  ...
  for (const [studentId, providerRef, amount] of payments) {
    ...
    const componentAmounts = invoice.components.map((component) => ({
      component,
      amountXof: Math.floor(
        (amount * component.amountXof) / invoice.totalAmount,
      ),
    }));
    let remainder =
      amount - componentAmounts.reduce((sum, row) => sum + row.amountXof, 0);
    componentAmounts.sort((a, b) =>
      a.component.kind.localeCompare(b.component.kind),
    );
  ...
  await prisma.applicant.createMany({
    data: [
      ...
      },
      {
        firstName: "Fatima",
        lastName: "Diallo",
        email: "fatima.diallo@example.gn",
        programCode: "BSCE",
        stage: "offer",
        score: 17,
        country: "Guinea",
      },
      {
        firstName: "Moussa",
        lastName: "Traoré",
        email: "moussa.traore@example.ml",
        programCode: "BSCE",
        stage: "accepted",
        score: 18,
        country: "Mali",
...
async function seedCampus() {
  if ((await prisma.event.count()) === 0) {
    await prisma.event.createMany({
      data: [
        {
          title: "Career Fair — Atlantic Atrium",
          description: "60+ employers on campus. Bring your CV.",
          location: "Atlantic Atrium",
          category: "Career",
        ...
        {
          title: "Intramural Football Final",
          description: "Cheer on your department.",
        ...
        },
        {
          ...
          description: "Music, poetry, and more.",
          location: "Student Center",
          category: "Arts",
          startsAt: new Date("2026-09-27T19:00:00Z"),
        },
      ],
    });
  }
  ...
  console.log(
    `Campus: ${await prisma.event.count()} events, ${await prisma.libraryResource.count()} library resources.`,
  );
}

async function seedMessages() {
  if ((await prisma.thread.count()) > 0) return;
  const aissatou = await prisma.person.findFirst({
    where: { email: "aissatou.diallo@daust.edu" },
  });
  const amadou = await prisma.person.findFirst({
    where: { email: "amadou.ba@daust.edu" },
  });
  ...
  const thread = await prisma.thread.create({
    data: {
      ...
      participants: {
        create: [{ personId: aissatou.id }, { personId: amadou.id }],
      },
    },
  });
  await prisma.message.create({
    data: {
      threadId: thread.id,
      senderId: aissatou.id,
      body: "Professor, may I schedule an advising session about my project?",
    },
  });
  await prisma.message.create({
    data: {
      threadId: thread.id,
      senderId: amadou.id,
      body: "Of course — does Thursday at 14:00 during office hours work?",
    },
  });
...
async function seedDining() {
  if ((await prisma.menuItem.count()) === 0) {
    await prisma.menuItem.createMany({
      data: [
        {
          name: "Thiéboudienne",
          description: "Senegalese rice & fish",
          category: "weekend",
          priceXof: 2500,
        },
        {
          name: "Yassa Poulet",
          description: "Grilled chicken, onion-lemon sauce",
          category: "weekend",
          priceXof: 2000,
        },
        {
          name: "Mafé",
          description: "Peanut stew with beef",
          category: "weekend",
          priceXof: 2200,
        },
        {
  ...
  const effectiveYears = await prisma.academicYear.findMany({
    where: {
      status: "active",
  ...
  if (effectiveYears.length !== 1) {
    throw new Error(
      "Dining seed requires one date-bounded active academic year",
    );
  }
  const effectiveYear = effectiveYears[0]!;
  // Give demo students annual meal plans (full for Aïssatou, half for Mamadou).
  const plans: [string, "full" | "half"][] = [
    ["stu_demo_aissatou", "full"],
    ["stu_mamadou", "half"],
  ];
      ...
      await prisma.mealPlan.upsert({
        where: {
          studentId_academicYearLabel: {
            studentId,
            academicYearLabel: effectiveYear.label,
          },
        },
  ...
  console.log(
    `Dining: ${await prisma.menuItem.count()} menu items, meal plans seeded.`,
  );
}
...
async function seedHousing() {
  ...
  if (!activeYear)
    throw new Error("Housing seed requires an active academic year");
  if ((await prisma.hall.count()) === 0) {
    await prisma.hall.createMany({
      data: [
        {
          name: "Teranga Hall",
          kind: "First-year · Mixed",
          beds: 320,
          color: "#153b6a",
        },
        {
          name: "Gorée Hall",
          kind: "Upper-year · Women",
          beds: 264,
          color: "#1d4a82",
        },
        {
  ...
  const teranga = await prisma.hall.findUnique({
    where: { name: "Teranga Hall" },
  });
  ...
  const housing: {
    ...
    {
      id: "stu_demo_aissatou",
      hallId: goree?.id ?? null,
      room: "G-214",
      status: "assigned",
    },
    {
      id: "stu_mamadou",
      hallId: teranga?.id ?? null,
      room: "T-118",
      status: "assigned",
    },
    {
    ...
    },
    {
      ...
      room: null,
      status: "pending",
      note: "International first-year · arrival soon",
    },
  ];
  ...
  console.log(
    "Housing: halls and room assignments seeded (backs the student Housing screen).",
  );
}

async function seedTrackD() {
  // Course materials + class posts for CE 201 (faculty course-detail tabs).
  const ce201 = await prisma.section.findFirst({
    where: { course: { code: "CE 201" }, term: { name: "Fall 2026" } },
  });
  if (
    ce201 &&
    (await prisma.sectionMaterial.count({ where: { sectionId: ce201.id } })) ===
      0
  ) {
    await prisma.sectionMaterial.createMany({
      data: [
        ...
        // student read path filters those out. The unpublished row exercises the gate.
        {
          ...
          category: "syllabus",
          sortOrder: 0,
          fileUrl: "/uploads/seed-ce201-syllabus.pdf",
          fileName: "ce201-syllabus.pdf",
          published: true,
        },
        {
          sectionId: ce201.id,
          title: "Week 1 — Boolean Algebra Slides",
          kind: "Slides",
          category: "lecture_notes",
          sortOrder: 1,
          fileUrl: "/uploads/seed-ce201-week1.pdf",
          fileName: "week1-boolean.pdf",
          published: true,
        },
        {
          sectionId: ce201.id,
          title: "Lab Manual — Digital Systems",
          kind: "Document",
          category: "assignments",
          sortOrder: 2,
          fileUrl: "/uploads/seed-ce201-lab-manual.pdf",
          fileName: "lab-manual.pdf",
          published: true,
        },
        {
          sectionId: ce201.id,
          title: "K-map Tutorial Video",
          kind: "Video",
          category: "resources",
          sortOrder: 3,
          fileUrl: "/uploads/seed-ce201-kmap.mp4",
          fileName: "kmap.mp4",
          published: false,
        },
      ],
    ...
    await prisma.sectionPost.createMany({
      data: [
        {
          sectionId: ce201.id,
          title: "Welcome to CE 201",
          body: "Syllabus is under Materials. First lab meets Thursday in R105.",
          author: "Amadou Ba",
          pinned: true,
        },
        {
          sectionId: ce201.id,
          title: "Midterm moved to Oct 20",
          body: "Same room, same duration — plan accordingly.",
          author: "Amadou Ba",
        },
      ],
    });
    ...
    await prisma.onboardingCase.createMany({
      data: [
        ...
        },
        {
          ...
          tasks: [
            { label: "Residency permit", done: false },
            { label: "Housing letter", done: false },
        ...
        },
        {
          ...
          origin: "Lyon, FR",
          kind: "Exchange",
          visaStatus: "Action needed",
          arrivalDate: new Date("2026-08-30"),
          tasks: [
            { label: "Housing letter", done: false },
            { label: "Permit appointment", done: false },
          ],
        },
        {
          name: "Kwame Mensah",
  ...
  if ((await prisma.abroadProgram.count()) === 0) {
    await prisma.abroadProgram.createMany({
      data: [
        {
          name: "MIT Summer Exchange",
          kind: "Study abroad",
          partner: "Cambridge, US",
          seatsTotal: 8,
          seatsTaken: 6,
        ...
        {
          name: "Siemens Engineering Internship",
          kind: "Internship",
          partner: "Munich, DE",
          seatsTotal: 4,
          seatsTaken: 3,
        ...
        {
          name: "Sonatel Data Science Co-op",
          kind: "Internship",
          partner: "Dakar, SN",
          seatsTotal: 12,
          seatsTaken: 12,
        ...
        {
          name: "Sorbonne Research Semester",
          kind: "Study abroad",
          partner: "Paris, FR",
          seatsTotal: 5,
          seatsTaken: 2,
  ...
  const baobab = await prisma.hall.findUnique({
    where: { name: "Baobab Hall" },
  });
      ...
      data: [
        {
          hallId: baobab.id,
          room: "B-106",
          kind: "AC unit unresolved",
          note: "Open 11 days — follow up with facilities.",
          severity: "low",
        },
        ...(sahel
          ? [
              {
                hallId: sahel.id,
                room: "S-303",
                kind: "Noise complaints ×2",
                note: "Two roommate-reported incidents this month.",
                severity: "med",
              },
            ]
          : []),
  ...
  if ((await prisma.globalTask.count()) === 0) {
    await prisma.globalTask.createMany({
      data: [
        ...
        {
          title: "Record a 2-min Pitch Video",
          kind: "Video",
    ...
    for (const t of tasks) {
      for (const pr of projects) {
        await prisma.projectGlobalTask.upsert({
          where: {
            globalTaskId_projectId: { globalTaskId: t.id, projectId: pr.id },
          },
          update: {},
          create: {
            globalTaskId: t.id,
            projectId: pr.id,
            done: t.title.includes("Proposal"),
          },
        });
      }
    }
  ...
  await prisma.event.updateMany({
    where: { title: { contains: "Career Fair" } },
    data: {
  ...
  await prisma.event.updateMany({
    where: { title: { contains: "Open Mic" } },
    data: {
      organizer: "Cultural Collective",
      attendees: 180,
      budgetXof: 1_000_000,
      status: "planning",
    },
  ...
  console.log(
    "Track D: materials/posts, onboarding, abroad, maintenance, global tasks seeded.",
  );
}
...
async function seedGuardians(passwordHash: string) {
  const children = await prisma.student.findMany({
    where: {
      person: {
        email: { in: ["aissatou.diallo@daust.edu", "mamadou.sy@daust.edu"] },
      },
    },
  });
  ...
  console.log(
    `Guardians: parent@daust.edu linked to ${children.length} student(s).`,
  );
}
...
async function seedInfirmary() {
  ...
  const consultations = [
    {
      ...
      visitType: "Walk-in",
      clinicalNotes:
        "Student reports frequent headaches during exam periods. Prescription updated.",
      status: "Completed",
      vitalsJson: {
        temperature: "36.8",
      ...
      treatmentPlan:
        "Increase water intake, reduce screen time, take Paracetamol as needed",
      followUpRequired: true,
      visitedAt: new Date("2026-08-21T09:42:00Z"),
    },
    {
      ...
      visitType: "Scheduled",
      clinicalNotes: "Routine check-up. All vitals normal.",
      status: "Completed",
      vitalsJson: {
        temperature: "36.6",
      ...
      visitedAt: new Date("2026-08-20T14:10:00Z"),
    },
    {
      ...
      visitType: "Follow-up",
      clinicalNotes:
        "Reviewing current inhaler usage. Symptoms well-controlled.",
      status: "Completed",
      vitalsJson: {
        temperature: "36.7",
      ...
      treatmentPlan: "Continue Salbutamol as needed. Avoid known triggers.",
      followUpRequired: true,
      visitedAt: new Date("2026-08-13T11:25:00Z"),
    },
  ];
    ...
    if (c.studentId === studentIds[0]) {
      await prisma.prescription.create({
        data: {
          ...
          frequency: "As needed",
          duration: "30 days",
          instructions: "Take 1-2 tablets every 6 hours. Max 4g/day.",
          status: "Active",
          prescribedAt: new Date("2026-08-21T10:00:00Z"),
        },
      });
    }
    if (c.studentId === studentIds[2]) {
      await prisma.prescription.create({
        data: {
          ...
          status: "Active",
          prescribedAt: new Date("2026-08-13T12:00:00Z"),
        },
      });
    }
  }
  console.log(
    `Seeded ${consultations.length} consultations with prescriptions.`,
  );
  ...
  const appointments = [
    {
      ...
      status: "Scheduled",
      notes: "Check if new medication regimen is working",
    },
    {
      studentId: studentIds[1],
  ...
  const followUps = [
    {
      ...
      status: "Pending",
      priority: "High",
      notes: "Check if Paracetamol is effective",
    },
    {
      studentId: studentIds[2],
      reason: "Asthma inhaler refill check",
      dueDate: new Date("2026-09-01"),
      status: "Pending",
      priority: "Medium",
      notes: "Verify inhaler stock",
    },
  ];
  ...
  const docs = [
    {
      studentId: studentIds[0],
      uploaderId: nurse?.id,
      name: "Migraine Treatment Plan",
      type: "Medical Record",
      notes: "Updated treatment plan for recurring migraines",
    ...
    },
    {
      ...
      type: "Medical Record",
      notes: "Annual asthma management plan",
  ...
  const form1 = await prisma.infirmaryForm.create({
    data: {
      ...
      description:
        "Collect essential health information before students arrive on campus.",
      questions: [
        {
          id: "Q1",
        ...
        {
          id: "Q5",
          text: "Rate your overall health (1-5)",
          type: "rating",
          required: true,
        },
        {
          id: "Q6",
          text: "Have you had any surgeries in the past 2 years?",
          type: "yes_no",
          required: true,
        },
      ],
  ...
  const form2 = await prisma.infirmaryForm.create({
    data: {
      ...
      questions: [
        {
          id: "Q1",
          text: "Which sport do you participate in?",
          type: "text",
          required: true,
        },
        {
          id: "Q2",
          text: "Have you had any sports injuries?",
          type: "yes_no",
          required: true,
        },
        {
          id: "Q3",
          text: "Do you have a heart condition?",
          type: "yes_no",
          required: true,
        },
        {
          id: "Q4",
          text: "Rate your fitness level (1-5)",
          type: "rating",
          required: true,
        },
        {
  ...
  const responses = [
    ...
    },
    {
      formId: form1.id,
      studentId: studentIds[1],
      studentName: "Mamadou Ndiaye",
      answers: { Q1: "No", Q2: "", Q3: "No", Q4: "", Q5: "5", Q6: "No" },
      submittedAt: new Date("2026-08-17T14:15:00Z"),
    },
    {
      formId: form2.id,
      studentId: studentIds[2],
      studentName: "Binata Sow",
      answers: { Q1: "Basketball", Q2: "No", Q3: "No", Q4: "4", Q5: "No" },
