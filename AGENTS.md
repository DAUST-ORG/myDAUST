...
| `CHANGELOG.md`                                                                                                 | **Generated by release-please.** Content is the marker block until the first release-please run; authoritative changelogs live in GitHub Releases (see `CONTRIBUTING.md` and §16)                                                                                                                                       |
...
The full convention — allowed types, the `(scope)` inventory, the `(AI-generated)` suffix, and the
`release:` promotion prefix — is documented in [`CONTRIBUTING.md`](./CONTRIBUTING.md). A standalone
checker lives at `scripts/check-commit-msg.mjs`; run it explicitly as
`pnpm commit:check "<subject>"` (no git hook is installed — see §17).
