  ...
  prerequisites: z
    .array(
      z.object({
        ...
        minGrade: z.string().max(4).nullish(),
      }),
    )
    .max(20),
...
export class RegistrarController {
  ...
  upsertDepartment(@CurrentUser() user: AuthUser, @Body() body: unknown) {
    return this.registrar.upsertDepartment(
      user.personId,
      DepartmentInput.parse(body),
    );
  }
  ...
  createAcademicYear(@CurrentUser() user: AuthUser, @Body() body: unknown) {
    return this.registrar.createAcademicYear(
      user.personId,
      AcademicYearInput.parse(body).label,
    );
  }
  ...
  updateGradeRow(
    ...
    return this.registrar.updateGradeRow(
      user.personId,
      rowId,
      GradeRowPatch.parse(body),
    );
  }
  ...
  setRule(
    ...
    return this.registrar.setCourseRule(
      user.personId,
      courseId,
      CourseRuleInput.parse(body),
    );
  }
  ...
  setRequisites(
    ...
    return this.registrar.setCourseRequisites(
      user.personId,
      courseId,
      RequisitesInput.parse(body),
    );
  }
  ...
  decide(
    ...
    return this.registrar.decideGradeSubmission(
      user.personId,
      id,
      input.decision,
      input.note,
  ...
  warn(@CurrentUser() user: AuthUser, @Body() body: unknown) {
    ...
    return this.registrar.warnStudent(
      user.personId,
      input.studentId,
      input.reason,
      input.level,
  ...
  createCalendarEvent(@CurrentUser() user: AuthUser, @Body() body: unknown) {
    return this.registrar.createCalendarEvent(
      user.personId,
      CalendarEventInput.parse(body),
    );
  }
  ...
  updateCalendarEvent(
    ...
    return this.registrar.updateCalendarEvent(
      user.personId,
      id,
      CalendarEventPatch.parse(body),
    );
  }
  ...
  saveCurriculum(@CurrentUser() user: AuthUser, @Body() body: unknown) {
    ...
    return this.registrar.saveCurriculum(
      user.personId,
      input.programCode,
