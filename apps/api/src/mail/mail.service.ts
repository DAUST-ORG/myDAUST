...
export interface MailMessage {
  ...
  subject: string;
  html: string;
  /** Stable key for safe retries through providers that support idempotency. */
...
export class MailService {
  ...
  private readonly from =
    process.env.MAIL_FROM ?? "myDAUST <no-reply@updates.daust.net>";

  async send(msg: MailMessage): Promise<{ sent: boolean; id?: string }> {
    const toLabel = Array.isArray(msg.to) ? msg.to.join(", ") : msg.to;
    if (!this.apiKey) {
      this.logger.log(
        `[dev-mail] to=${toLabel} subject="${msg.subject}" (no RESEND_API_KEY — not sent)`,
      );
      return { sent: false };
    }
