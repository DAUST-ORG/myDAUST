...
import { type EnrollmentGate, type EnrollmentGateFailure } from "@mydaust/shared";
import { bestPointsByCourse } from "../transcript/transcript-calculation.js";
import { MAX_CREDITS_PER_TERM, STANDING_RANK, meetingsOverlap, meetsPrerequisite } from "./academics.service.js";

/**
 * Run every waivable gate enroll() enforces, in the same order, returning the failures
 * instead of throwing. Pure: no writes, no side effects. The student-facing `enroll()` and
 * the override `applyEnrollmentOverride()` both call this, so the gate set cannot drift
 * between the two paths without breaking one of them at runtime.
 *
...
export async function evaluateEnrollmentGates(
  ...
  if (course.coreqRules.length > 0) {
    ...
    if (course.rule?.majorRestriction) {
      ...
      if (
        mine &&
        !tokenHeads.some((head) => mine.includes(head))
      ) {
        failures.push({
