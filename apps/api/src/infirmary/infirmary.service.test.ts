  ...
  return {
    ...
    infirmaryForm: {
      ...
      create: vi
        .fn()
        .mockImplementation(({ data }) =>
          Promise.resolve({ id: "form-1", ...data }),
        ),
      update: vi
        .fn()
        .mockImplementation(({ where, data }) =>
...
describe("InfirmaryService", () => {
  ...
  describe("Consultations", () => {
    ...
    it("getConsultation throws 404 for missing", async () => {
      ...
      await expect(svc.getConsultation("missing")).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("createConsultation sets clinicianId from actor", async () => {
      await svc.createConsultation(
        { studentId: "s-1", reason: "flu", visitType: "Walk-in" },
        ACTOR,
      );
    ...
    it("createConsultation writes audit log", async () => {
      await svc.createConsultation(
        { studentId: "s-1", reason: "test", visitType: "Routine" },
        ACTOR,
      );
    ...
    it("updateConsultation throws 404 for missing", async () => {
      prisma.consultation.findUnique.mockResolvedValue(null);
      await expect(
        svc.updateConsultation("missing", { reason: "x" }, ACTOR),
    ...
    it("deleteConsultation catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteConsultation("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("deleteConsultation catches P2003 as constraint error", async () => {
      ...
      await expect(svc.deleteConsultation("c-1", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
      await expect(svc.deleteConsultation("c-1", ACTOR)).rejects.toThrow(
    ...
    it("deleteConsultation rethrows non-P2025/P2003 errors", async () => {
      ...
      await expect(svc.deleteConsultation("c-1", ACTOR)).rejects.toMatchObject({
        code: "P1234",
      });
    });
  ...
  describe("Prescriptions", () => {
    ...
    it("deletePrescription catches P2025 as 404", async () => {
      ...
      await expect(svc.deletePrescription("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("updatePrescription throws 404 for missing", async () => {
      ...
      await expect(
        svc.updatePrescription("missing", {}, ACTOR),
      ).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe("Medications", () => {
    ...
    it("updateMedication throws 404 for missing", async () => {
      ...
      await expect(
        svc.updateMedication("missing", { stock: 50 }, ACTOR),
      ).rejects.toThrow(NotFoundException);
    });

    it("deleteMedication catches P2025 as 404", async () => {
      prisma.medication.delete.mockRejectedValue({ code: "P2025" });
      await expect(svc.deleteMedication("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });

    it("createMedication sets status Out of Stock when stock is 0", async () => {
      await svc.createMedication(
        { name: "Test", stock: 0, minStock: 5 },
        ACTOR,
      );
      const callArgs = prisma.medication.create.mock.calls[0][0];
      expect(callArgs.data.status).toBe("Out of Stock");
    });

    it("createMedication sets status Low Stock when stock < minStock", async () => {
      await svc.createMedication(
        { name: "Test", stock: 3, minStock: 10 },
        ACTOR,
      );
      const callArgs = prisma.medication.create.mock.calls[0][0];
      expect(callArgs.data.status).toBe("Low Stock");
    });

    it("createMedication sets status In Stock when stock >= minStock", async () => {
      await svc.createMedication(
        { name: "Test", stock: 100, minStock: 10 },
        ACTOR,
      );
      const callArgs = prisma.medication.create.mock.calls[0][0];
      expect(callArgs.data.status).toBe("In Stock");
    });

    it("createMedication coerces string stock to number", async () => {
      await svc.createMedication(
        { name: "Test", stock: "5", minStock: "10" },
        ACTOR,
      );
      const callArgs = prisma.medication.create.mock.calls[0][0];
      expect(callArgs.data.status).toBe("Low Stock");
    ...
    it("updateMedication recomputes status from new stock", async () => {
      prisma.medication.findUnique.mockResolvedValue({
        id: "m-1",
        stock: 50,
        minStock: 10,
      });
      await svc.updateMedication("m-1", { stock: 2 }, ACTOR);
      const callArgs = prisma.medication.update.mock.calls[0][0];
      expect(callArgs.data.status).toBe("Low Stock");
    });

    it("updateMedication keeps existing minStock when not provided", async () => {
      prisma.medication.findUnique.mockResolvedValue({
        id: "m-1",
        stock: 50,
        minStock: 100,
      });
      await svc.updateMedication("m-1", { stock: 0 }, ACTOR);
      const callArgs = prisma.medication.update.mock.calls[0][0];
      expect(callArgs.data.status).toBe("Out of Stock");
    });
  });
  ...
  describe("Appointments", () => {
    ...
    it("deleteAppointment catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteAppointment("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("Documents", () => {
    ...
    it("deleteDocument catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteDocument("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("Follow-ups", () => {
    it("createFollowUp writes audit", async () => {
      await svc.createFollowUp(
        { studentId: "s-1", reason: "Checkup", dueDate: new Date() },
        ACTOR,
      );
      const audit = prisma.auditLog.create.mock.calls[0][0];
    ...
    it("deleteFollowUp catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteFollowUp("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("Forms", () => {
    ...
    it("deleteForm catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteForm("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it("deleteForm catches P2003 (restrict) as constraint error", async () => {
      ...
      await expect(svc.deleteForm("form-1", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
      await expect(svc.deleteForm("form-1", ACTOR)).rejects.toThrow(
        "referenced by other data",
  ...
  describe("Form Responses", () => {
    ...
    it("deleteFormResponse catches P2025 as 404", async () => {
      ...
      await expect(svc.deleteFormResponse("missing", ACTOR)).rejects.toThrow(
        NotFoundException,
      );
    });
  });
