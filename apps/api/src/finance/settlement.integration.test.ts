...
async function makeInvoiceWithPendingPayment(total: number, paymentAmount: number) {
 ...
 */
async function claimAndApply(paymentId: string, amount: number): Promise<boolean> {
...
describe.skipIf(noDb)("settlement money path", () => {
  ...
  it("credits an invoice exactly once for a settled payment", async () => {
    const { invoice, payment } = await makeInvoiceWithPendingPayment(500_000, 300_000);
  ...
  it("credits once when two settlements race (two bursars approving at the same time)", async () => {
    const { invoice, payment } = await makeInvoiceWithPendingPayment(500_000, 250_000);
  ...
  it("marks the invoice paid only when the balance is fully covered", async () => {
    const { invoice, payment } = await makeInvoiceWithPendingPayment(400_000, 400_000);
  ...
  it("does not settle a payment that was already cancelled", async () => {
    const { invoice, payment } = await makeInvoiceWithPendingPayment(500_000, 200_000);
  ...
  it("enforces the remaining balance, so a stale approval cannot overpay", async () => {
    ...
    const { invoice, payment } = await makeInvoiceWithPendingPayment(500_000, 500_000);
  ...
  it("allows only one payable PI-SPI request per invoice", async () => {
    ...
    await prisma.piSpiRequest.create({
      data: { ...base, txId: `PIS${randomUUID().replace(/-/g, "").slice(0, 20)}`, status: "sent" },
    });
