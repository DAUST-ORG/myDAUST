              ...
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    setFeeErr("Online fee payment is unavailable right now — you can pay at the Office of Admissions.");
                  }
                }}
              >
                Pay application fee ({feeXof.toLocaleString("en-US")} FCFA)
              </button>
              <button className="btn btn-outline btn-lg" style={{ justifyContent: "center" }} onClick={close}>
                Later
              </button>
            </div>
            {feeErr && <div style={{ color: "#c0392b", fontSize: 13, marginTop: 10 }}>{feeErr}</div>}
          </div>
        ) : (
          <div
            style={{
