...
const AREA_LINKS: { role: string; href: string; label: string }[] = [
  ...
  { role: "dining", href: "/dining", label: "Dining Admin" },
...
export function Topbar({
  ...
  useEffect(() => {
    ...
    const saved =
      (localStorage.getItem("daust-theme") as "light" | "dark") ?? "light";
    setTheme(saved);
  ...
  const navHits: SearchHit[] = useMemo(
    () =>
      nav.flatMap((g) =>
        g.items
          .filter((i) => !i.disabled)
          .map((i) => ({ group: "Go to", label: i.label, href: i.href })),
      ),
    [nav],
  );
  ...
  const announcementsHref = ["student", "faculty", "admin"].includes(area)
    ? `/${area}/announcements`
    : null;
  ...
  const initials = me.name
    ...
    .join("")
    .toUpperCase();
  const areas = AREA_LINKS.filter((a) => me.roles.includes(a.role)).reduce<
    typeof AREA_LINKS
      ...
      >
        ...
        >
          ...
          >
            ...
            <span
              style={{ display: "block", fontSize: 11, color: "var(--fg3)" }}
            >
              {me.email}
            </span>
          </span>
        ...
        {open === "user" && (
          <div style={{ ...dropdownCard, width: 260 }}>
                ...
                {areas.map((a) => (
                  <button
                    ...
                    style={{
                      ...
                      cursor: "pointer",
                      fontSize: 13,
                      fontWeight: pathname.startsWith(a.href) ? 700 : 500,
                      color: "var(--fg1)",
                    }}
                  >
                    {a.label}
            ...
            >
              {theme === "dark" ? (
                <Sun size={15} color="var(--daust-orange)" />
              ) : (
                <Moon size={15} color="var(--daust-navy)" />
              )}
