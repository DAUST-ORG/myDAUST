...
import { type MenuItem, createMenuItem, getAdminMenu, toggleMenuItem } from "@/lib/api";
...
export default function MenusPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", category: "weekend", priceXof: 0 });
  ...
  async function add() {
    if (!form.name) return;
    await createMenuItem({ name: form.name, description: form.description || undefined, category: form.category, priceXof: form.priceXof });
    setForm({ name: "", description: "", category: "weekend", priceXof: 0 });
    load();
  }
  ...
  return (
    <>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "2fr 2fr 1fr 1fr auto", gap: 10, alignItems: "end", marginTop: 8 }}>
          ...
          <tbody>
            {items.map((m) => (
              <tr key={m.id}>
                ...
                <td><button onClick={() => toggleMenuItem(m.id).then(load)} style={{ fontSize: 12 }}>{m.available ? "Hide" : "Show"}</button></td>
              </tr>
            ))}
          </tbody>
        </table>
