...
export default function FacultySchedulePage() {
  ...
  useEffect(() => {
    getFacultySchedule().then(setItems).catch(() => {});
  }, []);
  ...
  return (
    <>
      ...
      <div className="card" style={{ overflowX: "auto" }}>
              ...
              {items
                ...
                .map((e) => {
                  ...
                  return (
                    <div
                      key={e.sectionId}
                      onClick={() => router.push(`/faculty/classes/${e.sectionId}`)}
                      style={{
                    ...
                    >
                      <div style={{ fontWeight: 700, fontSize: 12 }}>{e.code}</div>
                      <div style={{ fontSize: 11, color: "rgba(255,255,255,.9)", lineHeight: 1.2 }}>{e.title}</div>
                      <div style={{ fontSize: 10, color: "rgba(255,255,255,.8)", marginTop: 2 }}>{e.startTime}–{e.endTime} · {e.room ?? "—"}</div>
                    </div>
                  );
                })}
            </div>
        ...
        {items.length === 0 && <p className="muted" style={{ marginTop: 12 }}>No sections assigned.</p>}
      </div>
    </>
  );
}
