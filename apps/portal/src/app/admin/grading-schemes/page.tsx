...
export default function GradingSchemesPage() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      setActive((prev) =>
        s.some((x) => x.key === prev)
          ? prev
          : (s.find((x) => x.isDefault)?.key ?? s[0]?.key ?? ""),
      );
    } catch (e) {
      setError(
        e instanceof Error ? e.message : "Could not load grading schemes.",
      );
    }
  }, []);
  useEffect(() => {
  ...
  async function saveRow() {
    ...
    const payload = {
      ...
      countsTowardGpa: draft.countsTowardGpa,
      countsTowardCredits: draft.countsTowardCredits,
    };
    try {
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          scheme.isDefault ? (
            <Badge tone="success">Institution default</Badge>
          ) : undefined
        }
      />
        ...
        <table>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: "center" }}>Earns credit</th>
              <th style={{ width: 96 }} />
            </tr>
          </thead>
          <tbody>
            {scheme.rows.map((r) => (
              <tr key={r.id}>
                ...
                <td style={{ textAlign: "center" }}>
                  {r.countsTowardGpa ? "Yes" : "No"}
                </td>
                <td style={{ textAlign: "center" }}>
                  {r.countsTowardCredits ? "Yes" : "No"}
                </td>
                <td>
                  <div
                    style={{
      ...
      <p className="muted" style={{ fontSize: 12, marginTop: 14 }}>
        Scales without grade points (pass/fail, IEP levels) are excluded from
        GPA rather than counted as zero.
      </p>
      ...
      {removing && (
        <ConfirmDialog
          ...
          message={
            <>
              Delete the <strong>{removing.grade}</strong> row from{" "}
              {scheme.name}? Existing grades of {removing.grade} will lose their
              GPA-point mapping until a replacement row is added.
            </>
          }
          onClose={() => setRemoving(null)}
          onConfirm={async () => {
            await removeRow(removing.id);
            setRemoving(null);
          }}
        />
      )}
      ...
      {editor && (
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <Field
              label="GPA points"
              hint="Leave blank for a scale excluded from GPA."
            >
              <Input
                value={draft.points}
            ...
            >
              <input
                type="checkbox"
                checked={draft.countsTowardGpa}
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    countsTowardGpa: event.target.checked,
                  }))
                }
              />
              Count this grade toward cumulative GPA
            </label>
            <label
              style={{
                display: "flex",
            ...
            >
              <input
                type="checkbox"
                checked={draft.countsTowardCredits}
                onChange={(event) =>
                  setDraft((current) => ({
                    ...current,
                    countsTowardCredits: event.target.checked,
                  }))
                }
              />
              Award earned credits for this grade
            </label>
          </div>
        </Modal>
      )}
