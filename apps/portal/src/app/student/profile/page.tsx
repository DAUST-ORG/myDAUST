...
export default function StudentProfile() {
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          >
            ...
            {p.academicProgress.level && (
              <span
                style={{
                  padding: "3px 11px",
                  borderRadius: "var(--radius-pill)",
                  background: "rgba(237,132,37,.2)",
                  color: "#ffd6ac",
                  fontSize: 11.5,
                  fontWeight: 750,
                }}
              >
                Level {p.academicProgress.level.code}
              </span>
            )}
          </div>
          <div style={{ opacity: 0.85, fontSize: 13.5, marginTop: 2 }}>
            {p.program ?? "No programme"}
          </div>
          ...
          >
            ...
            <HeroStat
              label="Credits"
              value={`${p.completedCredits}${p.academicProgress.requiredCredits ? ` / ${p.academicProgress.requiredCredits}` : ""} earned`}
            />
            <HeroStat
              label="In progress"
              value={`${p.academicProgress.inProgressCredits} credits`}
            />
      ...
      {tab === "personal" && (
        <DetailCard title="Personal Information" fields={p.personal} />
      )}
      {tab === "academic" && (
      ...
      <p className="muted" style={{ fontSize: 12, marginTop: 16 }}>
        Your record is maintained by the registrar. Contact them to request a
        correction.
      </p>
    </>
  );
...
function RecordCard({
  ...
  return (
    ...
    >
      <dl
        style={{ margin: 0, display: "flex", flexDirection: "column", gap: 9 }}
      >
